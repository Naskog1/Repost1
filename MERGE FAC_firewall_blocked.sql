 ...
 using
(  select  f.*   , coalesce( [cc] , 'BG' ) as CC , getDate() as ETL_Insert , null as ETL_Update --- into [FAC_firewall_blocked]
from  ( 
		select  distinct [id],[timestamp] as timestamp_org  ,
		 CAST( replace([timestamp],',', '')  as datetime) as TimeStamp , CAST( Left([timestamp] ,10) as date) as TheDate ,
		message , 
